...
async function testLinkIngestServer(): Promise<void> {
  ...
  try {
    ...
    const response = await fetch('http://127.0.0.1:45124/api/link-batches', {
      method: 'POST',
    ...
    if (response.status !== 202 || !payload.ok) {
      throw new Error(`Unexpected server response: ${response.status} ${JSON.stringify(payload)}`);
    }
